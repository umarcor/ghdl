...
class Job(object):
    ...
    def start(self):
        self.p = subprocess.Popen(
            ["./testsuite.sh"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, close_fds=True, cwd=self.dirname
        )
...
def run(keep):
    ...
    while len(tests) != 0 or len(jobs) != 0:
        ...
        if len(tests) > 0 and len(jobs) < NUMJOBS:
            ...
            j = Job(test, poll)
            jobs.append(j)
            print("Starting {} [{}+{} err/{}]".format(test, nbr_run, nbr_err, nbr_tests))
            j.start()
        elif len(jobs) > 0:
    ...
    print("{}/{} tests run in {} sec, {} failures".format(nbr_run, nbr_tests, end_time - start_time, nbr_err))
    if failures:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-k", "--keep", help="keep running", action="store_true")
